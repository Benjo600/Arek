...
export function FlipButton({ text1, text2, className, onClick }: FlipButtonProps) {
  ...
  return (
      <div className={cn("group [perspective:1000px]", className)}>
        <motion.button
          className="relative w-full h-12 md:h-14 cursor-pointer rounded-xl font-bold text-base md:text-lg shadow-lg shadow-brand-500/20 hover:shadow-brand-500/40 transition-all outline-none focus:outline-none select-none"
          onClick={handleClick}
          initial={false}
